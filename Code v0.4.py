...
#Hyperparamètres
v = 1                     # Vitesse moyenne des bus (m/s)
f = 0.05                    # Fréquence des bus (bus/s)
c = 2
    ...
    def calcul_tps_trajet(self,chemin):
        ...
        for i in range(len(chemin)-2):
            ...
            if self.change_de_ligne(chemin[i:i+3]):
                value = 1/f - tps%(1/f)
                tps += value
                print("repere chgt ",value," -->",tps)
                #Simplification du modèle ici, autant de bus que d'arrêts
            print("repere 3 ",tps)
    ...
    def U_build(self):
        ...
        for l in self.lignes: #Pour chaque ligne
            ...
            for i in range(len(l.arrets)-1):
                print("UPDATE DISTANCE ",l.arrets[i].num,l.arrets[i+1].num)
